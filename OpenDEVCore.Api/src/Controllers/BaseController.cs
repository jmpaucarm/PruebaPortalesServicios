...
namespace OpenDEVCore.Api.Controllers
{
    ...
    public abstract class BaseController : ControllerBase
    {
        ...
        protected async Task<IActionResult> SendAsync<T>(T command, 
            Guid? resourceId = null, string resource = "", string session="") where T : ICommand 
        {
            var context = GetContext<T>(resourceId, resource, session);
        ...
        /// <param name="session"></param>
        /// <returns></returns>
        protected ICorrelationContext GetContext<T>(Guid? resourceId = null, string resource = "", string session = "") where T : ICommand
        {
            ...
            if (string.IsNullOrEmpty(session))
                ...
                return CorrelationContext.FromSession<T>(Guid.NewGuid(), session);
            else
